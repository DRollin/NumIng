...
# ╔═╡ 87f0248a-4bf9-11ee-1184-6fd53083587c
begin
	using PlutoUI, PlutoTeachingTools
	using Plots, ColorSchemes, ForwardDiff, LaTeXStrings
	...
	md"""
	# Nichtlineare Gleichungen: Fixpunkt-Iteration
...
# ╔═╡ f95ab76f-4a6f-42ad-aa7c-98f5bcf4a942
ChooseDisplayMode()

# ╔═╡ fefd0a42-46b4-4086-a314-cf80321aa99f
begin
	maxiter = 20
	x = -10:0.1:10
	possibleT = [
		begin x -> atan(x) +(x/3)^2-4 end => "atan(x) +(x/3)^2-4",
		begin x -> x^2 end => "x²",
		begin x -> 1.5^x-5 end => "1.5ˣ-5",
		begin x -> sin(x) + cos(x) end => "sin(x) + cos(x)"
	]
	...
	Betrachtete Fixpunkt-Form: ``T(x) =`` $(@bind T Select(possibleT)) ``= x`` für ``x \in [-10,10]``
	...
	- Startwert: ``x^{(0)} =`` $( @bind x₀ Select([-9,-7,-5,-2,0,2,5,7,9]; default=-9) )
	"""
end

# ╔═╡ 57f21f5c-6842-41ea-a1ff-b15f918279a8
begin
	xₙ = zeros(maxiter+1)
	xₙ[1] = x₀
	for n in 1:maxiter
...
# ╔═╡ 63c19e71-4f9e-4ac6-acf2-ee95ced9f9cd
begin
	p = plot(x, T.(x); label=L"T(x)", title="Fixpunkt-Iteration", framestyle=:zerolines, legend=:topleft, linewidth=2, size=(680,400), xlabel=L"x", ylabel=L"T")
	plot!(p, x, x; color=:grey, label=L"x")
	scatter!(p, xₙ[1:Nᵢₜₑᵣ+1], T.(xₙ[1:Nᵢₜₑᵣ+1]); label=false, color=:orange)
	
	for n in 1:Nᵢₜₑᵣ
		plot!(p, [xₙ[n], xₙ[n+1]], [T(xₙ[n]), xₙ[n+1]]; color=:green, arrow=true, label=false)
		plot!(p, [xₙ[n+1], xₙ[n+1]], [xₙ[n+1], T(xₙ[n+1])]; color=:grey, arrow=true, label=false)
	end
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "b68fa127af3de7a95b70d3006f4492cc79521ebb"
