...
begin
	...
	function perform_time_stepping(method::Function, f::Function, y₀::Real, tₑ::Real, Δt::Real)
		...
		y = zeros(length(t))
		y[1] = y₀
		tₛₒₗᵥₑ = @elapsed begin
			for n in 1:length(t)-1
				y[n+1] = method(f, y[n], t[n], t[n+1]-t[n])
			end
		end
		return t, y, tₛₒₗᵥₑ
	end
	
	struct MethodResults{T<:Real}
		name::String
		Δt::Vector{T}
		t::Vector{Vector{T}}
		y::Vector{Vector{T}}
	...
	function apply_methods(f::Function, methods, names, Δts, y₀, tₑ; yₑ=missing)
		results = MethodResults{Float64}[]
		for (method, name) in zip(methods, names)
			res = MethodResults{Float64}(name, Δts, [], [], zeros(length(Δts)), zeros(length(Δts)))
			for (i, Δt) in enumerate(Δts)
				t, y, tₛₒₗᵥₑ = perform_time_stepping(method, f, y₀, tₑ, Δt)
				push!(res.t, t)
				push!(res.y, y)
				if ! ismissing(yₑ)
					res.E[i] = abs(y[end] - yₑ)
				end
				res.tₛₒₗᵥₑ[i] = tₛₒₗᵥₑ
			end
			push!(results, res)
	...
	function plot_results(results, Δt; yᵣₑ=missing, tₑ=0)
		...
		for res in results
			...
			plot!(p, res.t[iΔt], res.y[iΔt]; label=res.name, linewidth=2)
		end
		return p
	end
...
begin
	...
	Es wird ein einfaches Anfangswert-Problem der Form

	##### ``\dot{y}(t) = f(y(t), t) ~ ; ~ t ∈ [0, t_e]``

	##### ``y(0) = y_0``

	gewählt, dessen Lösung bekannt ist:
	...
	Für die Einschrittverfahren
	
	- Euler vorwärts: ``y_{n+1} = y_n + \Delta t f(y_n, t_n)``
	- Euler rückärts: ``y_{n+1} = y_n + \Delta t f(y_{n+1}, t_{n+1})``
	- Trapez-Regel: ``y_{n+1} = y_n + \Delta t/2 [ f(y_n, t_n) + f(y_{n+1}, t_{n+1}) ]``

	ergeben sich daraus die folgenden Rechenvorschriften für einen Zeitschritt:

	- Euler vorwärts: ``y_{n+1} = y_n - \Delta t y_n``
	- Euler rückärts: ``y_{n+1} = \frac{ y_n }{ 1 + \Delta t }``
	- Trapez-Regel: ``y_{n+1} = \frac{ y_n - \Delta t/2 y_n }{ 1 + \Delta t/2 }``

	Außerdem ergibt sich für die 3/8-Regel:

	- ``k_1 = f(y_n, t_n) = -y_n``
	- ``k_2 = f(y_n + \frac{1}{3} \Delta t k_1 , t_n + \frac{1}{3} \Delta t) = -(y_n + \frac{1}{3} \Delta t k_1)``
	- ``k_3 = f(y_n + \frac{1}{3} \Delta t (-k_1 + 3k_2), t_n + \frac{2}{3} \Delta t) = -(y_n + \frac{1}{3} \Delta t (-k_1 + 3k_2))``
...
begin
	md"""
	$(plot_results(results₁, Δt₁; yᵣₑ=y, tₑ=10))
	"""
end
...
begin
	function euler_forward(f::Function, yₙ::Real, tₙ::Real, Δt::Real)
		return yₙ + Δt*f(yₙ, tₙ)
	end
	function euler_backward(f::Function, yₙ::Real, tₙ::Real, Δt::Real)
		tₙ₊₁ = tₙ + Δt
		problem = NonlinearProblem{false}( (yₙ₊₁, p) -> yₙ+Δt*f(yₙ₊₁,tₙ₊₁)-yₙ₊₁, yₙ)
		return solve(problem, NewtonRaphson(); reltol = 1e-9).u
	end
	function trapezoidal_rule(f::Function, yₙ::Real, tₙ::Real, Δt::Real)
		tₙ₊₁ = tₙ + Δt
		problem = NonlinearProblem{false}( (yₙ₊₁, p) -> yₙ+Δt/2*(f(yₙ,tₙ) + f(yₙ₊₁,tₙ₊₁))-yₙ₊₁, yₙ)
		return solve(problem, NewtonRaphson(); reltol = 1e-9).u
	end
	function runge_kutta_3_8(f::Function, yₙ::Real, tₙ::Real, Δt::Real)
		k₁ = f(yₙ, tₙ)
		k₂ = f(yₙ+Δt/3*k₁, tₙ+Δt/3)
		k₃ = f(yₙ+Δt/3*(-k₁+ 3*k₂), tₙ+2*Δt/3)
	...
	Es wird ein Anfangswert-Problem der Form
