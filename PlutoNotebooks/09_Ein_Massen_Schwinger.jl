...
begin
	...
	Es wird ein Zeitraum von t=0 bis t=tₑ betrachtet.
...
begin
	...
	function euler_forward_pendulum(xₙ::Real, vₙ::Real, tₙ::Real, Δt::Real)
		xₙ₊₁ = xₙ + Δt*vₙ
		vₙ₊₁ = vₙ + Δt*(F(tₙ)/m - δ2*vₙ - ω²*xₙ)
		return xₙ₊₁, vₙ₊₁
	end
	function euler_backward_pendulum(xₙ::Real, vₙ::Real, tₙ::Real, Δt::Real)
		tₙ₊₁ = tₙ + Δt
...
begin
	...
	for (method, name) in zip((euler_forward_pendulum, euler_backward_pendulum), ("Euler vorwärts", "Euler rückwärts"))
		...
		for n in 1:length(t)-1
			x[n+1], v[n+1] = method(x[n], v[n], t[n], Δt)
		end
		plot!(pₓ, t, x; label=name, linewidth=3)
		plot!(pᵥ, t, v; label=name, linewidth=3)
		plot!(pₚ, x, v; label=name, linewidth=3, arrows=true)
	end
