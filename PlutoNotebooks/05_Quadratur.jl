...
begin
	...
	plot!(pₜ, [xᵤ, xₒ], [f(xᵤ), f(xₒ)]; fillrange=0, fillalpha = 0.35, color=:green, linewidth=0)
	scatter!(pₜ, [xᵤ, xₒ], f.([xᵤ, xₒ]); color=:red, label=nothing)
	
	pₛ = hline([0.0]; label=nothing, color=:black, legend=nothing, title="Simpson-Regel", xlabel=md"``x``", ylabel=md"``f``")
	plot!(pₛ, x, f.(x); color=:blue, linewidth=2)
	plot!(pₛ, x, fₛ.(x); fillrange=0, fillalpha = 0.35, color=:green, linewidth=0)
	scatter!(pₛ, [xᵤ, (xᵤ+xₒ)/2, xₒ], f.([xᵤ, (xᵤ+xₒ)/2, xₒ]); color=:red)

	pg = hline([0.0]; label=nothing, color=:black, legend=nothing, title="Gauß-Quadratur mit 2 Punkten", xlabel=md"``x``", ylabel=md"``f``")
	plot!(pg, x, f.(x); color=:blue, linewidth=2)
	plot!(pg, x, fg.(x); fillrange=0, fillalpha = 0.35, color=:green, linewidth=0)
	hline!(pg, f.(xg); color=:gray, linewidth=1)
	scatter!(pg, xg, f.(xg); color=:red, label=nothing)
...
begin
	...
	Betrachtete Anzahlen Teilintervalle: ``N = 10^a`` mit ``a \in`` [$(@bind N_exp RangeSlider(N_exp_range; default=0:1))]

	Damit ist die Teilintervalllänge ``\frac{x_o-x_u}{N}`` und als Kleinheitsparameter kann ``\varepsilon = \frac{1}{N}`` genutzt werden.
	"""
end

# ╔═╡ ef08bf33-05c6-4733-a439-530bb27b229b
begin
	plot_range =  collect(N_exp_range[i] in N_exp for i in eachindex(N_exp_range))
	εₖ = 1 ./ Nₖ[plot_range]
	logεₖ = log10.(εₖ.^-1)
