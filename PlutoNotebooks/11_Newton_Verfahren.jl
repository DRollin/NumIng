...
begin
	...
	md"""
	# Nicht-Lineare Gleichungen: Newton- und Sekanten-Verfahren
...
# ╔═╡ 6b146e6d-bc72-410c-a3ea-be14c8319e1d
ChooseDisplayMode()
...
begin
	...
	x = -10:0.1:10
	possiblef = [
		begin x -> cos(x) + x^2 -10 end => "cos(x) + x^2 - 10",
		begin x -> atan(x) end => "atan(x)",
		begin x -> x^2-3 end => "x² - 3",
	]
	
	md"""
	## Lösen einer Gleichung in Nullstellen-Form

	Betrachtete Gleichung: ``f(x) =`` $(@bind f Select(possiblef)) ``= 0`` für ``x \in [-10,10]``

	Zum Lösen der Gleichung werden das Newton- und das Sekanten-Verfahren verwendet mit. Die Iterationsvorschriften der Verfahren lauten:
...
begin
	...
	- Startwert: ``x^{(0)}`` = $( @bind x₀ Select([-9,-5,-2,-1,0,1,2,5,9]; default=9) )
	- Sekanten-Verfahren: ``x^{(-1)}`` = $( @bind x₋₁ Select([-10,-9,-5,-2,-1,0,1,2,5,9,10]; default=10) )
	"""
end
...
begin
	...
	md"""
	$(plot(pₙ, pₛ; layout=(2,1), size=(680, 350*2)))
	"""
end
