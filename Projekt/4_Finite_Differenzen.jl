...
# ╔═╡ fd63b71e-a346-4226-b463-e56698b92347
begin
...
# ╔═╡ 8c94cd6a-6a87-4cc1-ae7c-e66a0c4e4536
ChooseDisplayMode()
...
# ╔═╡ a8a652c3-16b1-42b8-a89a-a2210a8710ad
begin
	trapez(xᵤ, xₒ, fᵤ, fₒ) = (fᵤ + fₒ)/2 * (xₒ-xᵤ)
	simpson(xᵤ, xₘ, xₒ, fᵤ, fₘ, fₒ) = (fᵤ + 4*fₘ + fₒ)/6 * (xₒ-xᵤ)
	qrs = ["Trapez-Regel", "Simpson-Regel"]

	md"""
	- **Quadratur**: Es wurden für die Formänderunsenergie ``\Pi = \int \frac{1}{2} \frac{M^2}{EI}`` eines Balkens unter reiner Biegung Quadratur-Regeln untersucht. Im Folgenden wird eine Quadratur-Regel benötigt, die äquidistante Stützsetellen nutzt, damit die Stützstellen der Finiten Differenzenmethode verwendet werden können. Es wird die $(@bind quadraturerule Select(qrs; default="Simpson-Regel")) angewendet.
	"""
end
...
begin
	function prepare_and_solve_system(N::Integer, qr::String, F::Real)
		...
		function κ(i) 
			if i == 1
				return (wᵢ[i] - 2*wᵢ[i+1] + wᵢ[i+2])/(Δx^2)
			elseif i == nnodes
				return (wᵢ[i-2] - 2*wᵢ[i-1] + wᵢ[i])/(Δx^2)
			else
				return (wᵢ[i-1] - 2*wᵢ[i] + wᵢ[i+1])/(Δx^2)
			end
		end
...
# ╔═╡ 4598f75d-3386-4863-b117-ecc12b597f06
begin
	md"""
	zusammen mit dem Lastvektor:
	"""
end

# ╔═╡ a6c9f17e-5fa4-4eb8-9a45-3810fb97a84f
begin
	bₘ
end
...
begin
	...
	Zur Berechnung der Druchbiegung wird die Gleichung ``w'' = -\frac{1}{EI}M`` durch ``\frac{w_{i-1} - 2w_{i} + w_{i+1}}{\Delta x^2} = -\frac{1}{EI(x_{i})}M_{i}`` für ``1 < i < N`` diskretisiert.
...
# ╔═╡ c7876d02-28c2-4a2e-859f-f9a81062f47e
begin
	Aw
end

# ╔═╡ 9c7ed688-3758-460c-96b6-ee6ef26639a9
begin
...
begin
	...
	md"""
	Nach Lösen des Gleichungssystems ergeben sich folgende Durchbiegungen an den Stützstellen:
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "8be17a25eb2b521f263a1946cbaf2d294ec751b7"
